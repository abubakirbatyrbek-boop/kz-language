...
function localProgress(){try{return JSON.parse(localStorage.getItem('v5_progress')||'{}')}catch{return {}}}
function saveLocal(k,v){const p=localProgress();p[k]=v;localStorage.setItem('v5_progress',JSON.stringify(p))}
function getUser(){return window.__v5User||null}
async function initUser(){try{if(window.supabase&&window.SUPABASE_CONFIG?.url){const c=window.supabase.createClient(window.SUPABASE_CONFIG.url,window.SUPABASE_CONFIG.publishableKey);window.__v5Supabase=c;const {data}=await c.auth.getUser();window.__v5User=data?.user||null;if(window.__v5User){const {data:rows}=await c.from('learning_progress').select('node_id,language,status,score,updated_at').eq('user_id',window.__v5User.id).limit(1000);if(Array.isArray(rows)&&rows.length){const p=localProgress();rows.forEach(r=>{p[r.node_id]= {language:r.language,status:r.status,score:r.score,updated_at:r.updated_at}});localStorage.setItem('v5_progress',JSON.stringify(p))}}}}catch(e){console.warn('auth/progress init failed',e)}return getUser()}
async function saveRemoteProgress(nodeId,data){saveLocal(nodeId,data);const u=getUser(),s=window.__v5Supabase;if(!u||!s)return;try{await s.from('learning_progress').upsert({user_id:u.id,node_id:nodeId,language:data.language,status:data.status||'in_progress',score:data.score??null,updated_at:new Date().toISOString()},{onConflict:'user_id,node_id'})}catch(e){console.warn('remote progress failed',e)}}
async function saveTestResult(nodeId,data){const u=getUser(),s=window.__v5Supabase;if(!u||!s)return;try{await s.from('test_results').insert({user_id:u.id,node_id:nodeId,language:data.language,score:data.score,passed:data.passed,answers:data.answers||[],created_at:new Date().toISOString()})}catch(e){console.warn('remote test failed',e)}}
...
function unitUnlocked(path,idx){if(idx===0)return true;return !!calcUnitState(path.units[idx-1]).test}
...
function renderPath(){const path=getPath();document.title=`${path.label}学习路径｜中亚语言通`;document.getElementById('pathTitle').textContent=`${path.flag} ${path.label}`;document.getElementById('pathDesc').textContent=path.desc;const unlocked=path.units.filter((u,i)=>unitUnlocked(path,i)).length;document.getElementById('pathStats').innerHTML=`<div><b>${unlocked}</b><span>已解锁单元</span></div><div><b>${path.units.length}</b><span>总单元</span></div><div><b>${path.units.reduce((a,u)=>a+u.lessons.length,0)}</b><span>小课</span></div>`;document.getElementById('pathList').innerHTML=path.units.map((u,i)=>{const open=unitUnlocked(path,i),st=calcUnitState(u);return `<div class="path-node ${open?'open':'locked'}"><div class="node-num">${open?u.num:'🔒'}</div><div class="node-main"><div class="node-top"><span class="eyebrow">UNIT ${u.num} · ${u.level}</span><span>${st.test?'✅ 已通过':st.percent+'%'}</span></div><h3>${u.title}</h3><p>${u.desc}</p><div class="progress-track"><span style="width:${st.percent}%"></span></div></div><div class="node-action">${open?`<a class="primary-btn small" href="unit.html?lang=${langKey()}&unit=${u.id}">${st.percent?'继续':'开始'} →</a>`:`<span class="lock-copy">通过上一单元考试后解锁</span>`}</div></div>`}).join('')}
function renderUnit(){const path=getPath(),unitId=qs('unit')||path.units[0].id,unit=path.units.find(u=>u.id===unitId);if(!unit)return location.href=`path.html?lang=${langKey()}`;if(!unitUnlocked(path,path.units.indexOf(unit)))return location.href=`path.html?lang=${langKey()}`;document.getElementById('backLink').href=`path.html?lang=${langKey()}`;const st=calcUnitState(unit);document.getElementById('unitHeader').innerHTML=`<span class="eyebrow">UNIT ${unit.num} · ${unit.level}</span><h1>${unit.title}</h1><p>${unit.desc}</p><div class="unit-meter"><span>${st.done}/${st.total} 小课完成</span><div class="progress-track"><span style="width:${st.percent}%"></span></div></div>`;document.getElementById('lessonList').innerHTML=unit.lessons.map((l,i)=>{const done=localProgress()[nodeKey(l.id)]?.status==='done';const typeLabel={intro:'认识发音',listen:'听音选择',select:'认识词语',translate:'翻译',reorder:'组句',write:'造句'}[l.type]||'练习';return `<a class="lesson-row ${done?'done':''}" href="lesson-v4.html?lang=${langKey()}&unit=${unit.id}&lesson=${l.id}"><div class="lesson-index">${done?'✓':i+1}</div><div><strong>${l.title}</strong><span>${typeLabel}</span></div><b>${done?'已完成':'开始 →'}</b></a>`}).join('')+`<div class="unit-test-card"><div><span class="eyebrow">UNIT TEST</span><h3>单元考试</h3><p>必须先完成本单元所有小课，再参加测试；达到 80% 才能解锁下一单元。</p></div><a class="primary-btn" href="quiz-v4.html?lang=${langKey()}&unit=${unit.id}">参加考试 →</a></div>`}
function renderLesson(){const path=getPath(),unit=path.units.find(u=>u.id===qs('unit')),lesson=unit?.lessons.find(l=>l.id===qs('lesson'));if(!lesson)return;document.getElementById('lessonTop').innerHTML=`<a class="back-link" href="unit.html?lang=${langKey()}&unit=${unit.id}">← ${unit.title}</a><span class="eyebrow">小课 · ${unit.num}</span><h1>${lesson.title}</h1><p>从最简单的一步开始，完成本小课后再进入下一项。</p>`;const area=document.getElementById('exerciseArea');let i=0,score=0;const items=lesson.items;function finish(){const pct=Math.round(score/items.length*100);saveRemoteProgress(lesson.id,{language:langKey(),status:'done',score:pct});area.innerHTML=`<div class="result-card"><span class="eyebrow">完成</span><h2>本小课完成！</h2><div class="score-big">${pct}%</div><p>你完成了 ${items.length} 个练习。</p><a class="primary-btn" href="unit.html?lang=${langKey()}&unit=${unit.id}">返回单元 →</a></div>`}
...
if(lesson.type==='intro'){area.innerHTML=meta+`<div class="exercise-card pronunciation-card"><span class="eyebrow">认识发音</span><div class="sound-symbol">${it.symbol}</div><h2>${it.example}</h2><p class="meaning">${it.meaning}</p><p class="hint">${it.note}</p><button class="primary-btn" id="playSound">🔊 听示范</button><button class="secondary-btn" id="knowIt">我会了，下一张 →</button></div>`;area.querySelector('#playSound').onclick=()=>speak(it.example,langKey());area.querySelector('#knowIt').onclick=()=>{score++;i++;next()};return}
